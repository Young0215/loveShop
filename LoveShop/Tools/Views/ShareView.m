...
//    }];
    ...
    [self.collection mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(SCREEN_SIZE.width, CURRENT_SIZE(60)));
        make.centerX.equalTo(weakSelf);
...
- (UILabel *) titleLabel{
    if(!_titleLabel){
        ...
        _titleLabel.frame = CGRectMake((SCREEN_WIDTH - 80)/2, 15, 80, 16);
...
- (UIButton *) cancleBtn{
    if(!_cancleBtn){
        ...
        _cancleBtn.frame = CGRectMake(0, 100, SCREEN_WIDTH, 25);
        [_cancleBtn setTitle:@"取消分享" forState:UIControlStateNormal];
        ...
        [_cancleBtn setTitleColor:KGrayColor forState:UIControlStateNormal];
        [_cancleBtn addTarget:self action:@selector(tapOnBgView) forControlEvents:UIControlEventTouchUpInside];
...
- (UICollectionView *) collection{
    if(!_collection){
        _collection = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 35, SCREEN_WIDTH, 60) collectionViewLayout:[[ShareCollectionViewFlowLayout alloc] init]];
