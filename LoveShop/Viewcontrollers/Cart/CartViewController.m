...
@interface CartViewController ()<UIWebViewDelegate>
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSURL *url = [NSURL URLWithString:ShoppingCar_URL];//创建URL
...
//        NSLog(@"订单号 金额 %@",strDic);
//        NSString *appScheme = @"loveShopPay";
//        Order *order = [[Order alloc] init];
//        order.partner = PARTNER;
//        order.sellerID = SELLER;
//        order.outTradeNO = [self generateTradeNO]; //订单ID（由商家自行制定）
//        order.subject = @"福满多"; //商品标题 [strDic objectForKey:@"out_trade_no"]
//        order.totalFee = @"0.01"; //商品价格 [strDic objectForKey:@"total_fee"]
...
//        __weak FenLeiViewController* wself = self;
        [[AlipaySDK defaultService]payUrlOrder:urlOrder fromScheme:@"loveShopPay" callback:^(NSDictionary* result) {
            // 处理支付结果
            NSLog(@"支付结果%@", result);
